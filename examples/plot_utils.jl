...
"""
Check if the gradient using finite differences matches the grad_iip! provided.
"""
function check_gradients(grad_iip!, f, gradient, num_tests=10, tolerance=1.0e-5)
    for i in 1:num_tests
        random_point = rand(length(gradient))
        grad_iip!(gradient, random_point)
