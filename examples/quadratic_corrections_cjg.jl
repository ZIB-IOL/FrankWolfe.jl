...
function cg_is_solve(x, A_mat, b)
    _, ch = IterativeSolvers.cg!(x, A_mat, b; abstol=1e-16, reltol=1e-16, maxiter=1000, log=true)
    return ch.isconverged
end

function lu_solve(x, A_mat, b)
    lu_fact = lu(A_mat)
    copyto!(x, lu_fact \ b)
    return issuccess(lu_fact)
end
    
function cholesky_solve(x, A_mat, b)
    cholesky_fact = cholesky(A_mat)
    copyto!(x, cholesky_fact \ b)
    return true
end

function gmres_is_solve(x, A_mat, b)
    _, ch = IterativeSolvers.gmres!(x, A_mat, b; abstol=1e-16, reltol=1e-16, maxiter=1000, log=true)
    return ch.isconverged
end
...
function is_solve(x, A_mat, b)
    _, ch = IterativeSolvers.solve!(x, A_mat, b; abstol=1e-16, reltol=1e-16, maxiter=1000, log=true)
    return ch.isconverged
end


solvers = [
    FrankWolfe.SymmetricAffineMinSolver(cg_solve),
    FrankWolfe.SymmetricAffineMinSolver(cg_is_solve),
    FrankWolfe.SymmetricAffineMinSolver(lu_solve),
    #FrankWolfe.SymmetricAffineMinSolver(cholesky_solve),
    FrankWolfe.SymmetricAffineMinSolver(gmres_is_solve),
    FrankWolfe.SymmetricAffineMinSolver(solve),
    FrankWolfe.NonSymmetricAffineMinSolver(solve),
    FrankWolfe.NonSymmetricAffineMinSolver(gmres_is_solve),
    true,
]
...
xtick_labels = ["CG", "CG is", "LU","GMRES is", "Solve", "Asym Solve", "Asym GMRES is", "Highs"]
ytick_labels = [
    "KSparse(5, 500)",
    "KSparse(5, 1)",
    "KSparse(10, 500)",
