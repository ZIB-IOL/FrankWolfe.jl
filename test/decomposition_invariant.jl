...
@testset "DICG standard run" begin
    ...
    @testset "LMO: $lmo" for lmo in (FrankWolfe.UnitSimplexOracle(1.0), FrankWolfe.ProbabilitySimplexOracle(1.0))
        x0_simplex = FrankWolfe.compute_extreme_point(lmo, randn(n))
        res_di = FrankWolfe.decomposition_invariant_conditional_gradient(f, grad!, lmo, x0_simplex, verbose=false, trajectory=true, epsilon=1e-10)
        res_fw = FrankWolfe.frank_wolfe(f, grad!, lmo, x0_simplex, verbose=false, trajectory=true, epsilon=1e-10)
