...
analyse_data_A = []
analyse_data_D = []
limit_num_steps_secant=60
file_name = joinpath(@__DIR__, "generlized_self_concordance_with_secant_analsye_data_" * string(limit_num_steps_secant) * ".csv")
...
@testset "Limit Optimal Design Problem" begin
    @testset "A-Optimal Design" begin 
        ...
        domain_oracle = build_domain_oracle(A)
        line_search=FrankWolfe.Secant(domain_oracle=domain_oracle, limit_num_steps=limit_num_steps_secant)
        x_s, _, primal, dual_gap, traj_data_s, _ = FrankWolfe.blended_pairwise_conditional_gradient(f, grad!, lmo, active_set, verbose=true, line_search=line_search, trajectory=true)
        ...
        df = DataFrame(criterion="A", seed=seed, dimension=m, number_not_converging=line_search.number_not_converging, fw_iterations=traj_data_s[end][1], ratio=line_search.number_not_converging/traj_data_s[end][1], improvement_by_backtracking=line_search.best_improvement_by_backtracking, max_violation=line_search.max_violation)
        CSV.write(file_name, df, append=true, writeheader= m == 100 && seed==1)
    ...
    @testset "D-Optimal Design" begin
        ...
        df = DataFrame(criterion="D", seed=seed, dimension=m, number_not_converging=line_search.number_not_converging, fw_iterations=traj_data_s[end][1], ratio=line_search.number_not_converging/traj_data_s[end][1], improvement_by_backtracking=line_search.best_improvement_by_backtracking, max_violation=line_search.max_violation)
        CSV.write(file_name, df, append=true)

        push!(analyse_data_D, (seed, m, line_search.number_not_converging, traj_data_s[end][1], line_search.number_not_converging/traj_data_s[end][1], line_search.best_improvement_by_backtracking))
