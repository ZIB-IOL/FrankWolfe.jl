...
Creates an `ActiveSetQuadraticLinearSolve` from the given Hessian `A`, linear term `b` and `lp_optimizer` by creating an inner `ActiveSetQuadraticCachedProducts` active set.
"""
function ActiveSetQuadraticLinearSolve(
    tuple_values::Vector{Tuple{R,AT}},
    A::H,
    ...
    inner_as = ActiveSetQuadraticCachedProducts(tuple_values, A, b)
...
function ActiveSetQuadraticLinearSolve{AT,R}(
    ...
    inner_as = ActiveSetQuadraticCachedProducts{AT,R}(tuple_values, A, b)
