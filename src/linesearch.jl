...
code and adapted.
"""
struct UnsafeGoldenratio{T} <: LineSearchMethod
    tol::T
end
...
Base.print(io::IO, ::UnsafeGoldenratio) = print(io, "UnsafeGoldenratio")

"""
    Goldenratio(tol=1e-7)

Like `UnsafeGoldenratio` but with numerically safer reconstruction of final step size `γ`.

Why:
- `UnsafeGoldenratio` computes bracket `[left, right]` in primal space, then reconstructs `γ` via a
...
`Goldenratio` computes `γ` using all coordinates via a projection formula and clamps it to
`[0, gamma_max]`.
"""
...
function perform_line_search(
    ...
    # More stable: error in UnsafeGoldenratio scales as 1/|dᵢ|, error in Goldenratio scales as 1/||d||₂, and
    #   |dᵢ| = |<d, eᵢ>| ≤ ||d||₂⋅||eᵢ||₂ = ||d||₂
    den = dot(d, d)
