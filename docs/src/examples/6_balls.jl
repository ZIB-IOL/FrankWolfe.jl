...
# # FrankWolfe for scaled, shifted ``L^1`` and ``L^{\infty}`` norm balls

# In this example, we run the vanilla FrankWolfe algorithm on a scaled and shifted ``L^1`` and ``L^{\infty}`` norm ball, using the `ScaledBoundL1NormBall`
...
println("\nFinal approximation: ", x_linf)


# We plot the polytopes alongside the approximative solutions from above:
